...
def comprobar_parametro(posicion,estado,fecha):
...
def validacion_parametro_opcional(estado,fecha):  
    if (len(sys.argv)==6):
        comprobar_parametro(5,estado,fecha)
    elif (len(sys.argv)==7): 
        comprobar_parametro(6,estado,fecha)
...
def procesar_datos(archivo,dni,salida,tipo,estado,fecha):
    ...
    for row in csvFile:  #para cada linea le agrego la linea correspondiente a mi lista si cumple con los filtros
        if(row['DNI']==dni and row['Tipo'].lower()==tipo and ((estado != None and row["Estado"] == estado) or estado == None)):
    ...
    if salida == "pantalla":
        ...
        campos_header = ['NroCheque','CodigoBanco','CodigoSucursal','NumeroCuentaOrigen','NumeroCuentaDestino','Valor','FechaOrigen','FechaPago','DNI','Tipo','Estado']
        writer = csv.DictWriter(nuevo_archivo, delimiter=",", fieldnames=campos_header)
        writer.writeheader()
...
if __name__=='__main__': #Entra en este if cuando apenas se ejecuta el codigo
    ...
    validacion_parametro_opcional(estado,fecha)
    ...
    procesar_datos(archivo,dni,salida,tipo_cheque,estado,fecha)